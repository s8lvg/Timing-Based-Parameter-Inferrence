...
def profile(session):
    ...
    for layer_cnt in range(1,MAX_NUM_LAYERS+1):
        ...
        network_loaded = load_network(session,f"models/iris_{layer_cnt}_relu_cross-entropy.onnx")
        print(f"[+] Loaded Netowork: {network_loaded}")
        # Measure NUM_MEASUREMENTS times with random inputs
        for _ in range(NUM_MEASUREMENTS):
            layers_to_timing[layer_cnt].append(random_parameters_request(session)['prediction_time'])
...
def main():
    ...
    y = np.array(flatten([[i]*NUM_MEASUREMENTS for i in range(1,MAX_NUM_LAYERS+1)]))


    print("[+] Training SVC")
    clf = make_pipeline(StandardScaler(), LogisticRegression(random_state=42,multi_class="ovr",n_jobs=16,verbose=1,solver='saga'))
    clf.fit(x,y)
